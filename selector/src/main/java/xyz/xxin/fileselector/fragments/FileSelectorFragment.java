package xyz.xxin.fileselector.fragments;
...
public class FileSelectorFragment extends Fragment {
    ...
    private void findView(View view) {
        file_selector_view = view.findViewById(R.id.file_select_view);
    }
    ...
    private void finishFragment() {
        ...
        if (activity != null) {
            ...
            fragmentManager.beginTransaction()
                    .remove(FileSelectorFragment.this)
                    .commit();
    ...
    private void loadListData(FileItemBean openFileItemBean, int loadType) {
        ...
        // 加载列表数据
        file_selector_view.loadListData(openFileItemBean, loadType, configBean.activity);
    }
        ...
        file_selector_view.setTitleBarButtonClickListener(new FileSelectorTitleBar.OnTitleClickListener() {
            ...
            public void onSelectClick(View view) {
                if (file_selector_view.isSelectMode()) {
                    ...
                    Toast.makeText(getContext(), "文件正在加载中", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
    ...
    private void initView() {
        // 设置状态栏
        StatusBarUtils.setStatusBarColor(configBean.activity, configBean.statusBarColorId);
        StatusBarUtils.setStatusTextColor(configBean.activity, configBean.statusBarIsDarkText);
        // 设置底部虚拟导航栏颜色
        StatusBarUtils.setNavigationBarColor(configBean.activity, configBean.navigationBarColorId);
        ...
        if (fragmentActivity != null) {
            fragmentActivity.getOnBackPressedDispatcher().addCallback(getViewLifecycleOwner(),
                    new CustomOnBackPressedCallback(configBean.isEnableOnBackPressedListener));
        }
    }
     ...
     * 返回按键监听回调
     */
    private class CustomOnBackPressedCallback extends OnBackPressedCallback {

        public CustomOnBackPressedCallback(boolean enabled) {
            super(enabled);
        }
    ...
    private boolean permissionManager() {
        boolean writeExternal = PermissionUtil.isWriteExternal(getContext());
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == Code.REQUEST_EXTERNAL && PermissionUtil.isWriteExternal(getContext())) {
            initList();
        }
    }
}